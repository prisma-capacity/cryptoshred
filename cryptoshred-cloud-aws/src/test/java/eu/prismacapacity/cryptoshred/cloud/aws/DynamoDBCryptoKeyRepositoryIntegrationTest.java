...
class DynamoDBCryptoKeyRepositoryIntegrationTest {
	...
	public static void setUp() {
		...
		client.createTable(new CreateTableRequest().withTableName(TABLE_NAME)
				...
				.withAttributeDefinitions(new AttributeDefinition().withAttributeName("subjectId")
	...
	class RaceConditionCryptoKeyRepository extends DynamoDBCryptoKeyRepository {
		...
		public RaceConditionCryptoKeyRepository(@NonNull CryptoEngine engine, @NonNull AmazonDynamoDB dynamoDB,
				@NonNull CryptoMetrics metrics, @NonNull String tableName, CryptoKey key) {
	...
	private void addExampleKey(CryptoSubjectId subjectId, CryptoAlgorithm algorithm, CryptoKeySize size,
			CryptoKey key) {
		val client = getClient();

		val item = new HashMap<String, AttributeValue>();
		item.put("subjectId", new AttributeValue(subjectId.getId().toString()));
		item.put(Utils.generateKeyPropertyName(algorithm, size),
