...
/**
 * CryptoKeyRepository implementation based on AWS DynamoDB.
 * Supports multiple keys (algorithm + size) for the same subject
 *
 * @author otbe
 *
 */
...
public class DynamoDBCryptoKeyRepository implements CryptoKeyRepository {
    ...
    @Override
    public Optional<CryptoKey> findKeyFor(@NonNull CryptoSubjectId subjectId, @NonNull CryptoAlgorithm algorithm, @NonNull CryptoKeySize size) {
        val getRequest = GetCryptoKeyRequest.of(subjectId, algorithm, size, tableName);

        val item = dynamoDB
                .getItem(getRequest.toDynamoRequest())
                .getItem();
    ...
    @Override
    public CryptoKey getOrCreateKeyFor(@NonNull CryptoSubjectId subjectId, @NonNull CryptoAlgorithm algorithm, @NonNull CryptoKeySize size) throws CryptoKeyNotFoundAfterCreatingException {
        val item = findKeyFor(subjectId, algorithm, size);

        return item.orElseGet(() -> createCryptoKey(subjectId, algorithm, size));
    ...
    private CryptoKey createCryptoKey(CryptoSubjectId subjectId, CryptoAlgorithm algorithm, CryptoKeySize size) {
        ...
        try {
            ...
            if (!resultKey.isPresent()) {
                // should never ever happen because that would indicate a broken DynamoDB API contract
            ...
            // this happens when the key was not found in the first step but someone created one in the meantime
            // the updateItem call checks that the key for algorithm and size does not exist before updating/creating the item
